...
### 4.2. 우아한 종료 메커니즘 (Graceful Shutdown)
...
### 4.3. 실시간 진행률 추적 (Real-time Progress Tracking)
...
### 4.4. 무상태 "최근 결과" 보기 (Stateless Inspection)
...
```bash
B2_crawling/
├── data/                  # 데이터 지속성 계층 (JSON, JSONL)
├── frontend/              # Single Page Application
│   └── index_v2.html      # 모던 검색 UI (Grid, Modal, Dashboard)
├── src/
│   ├── api_server.py      # FastAPI 엔트리포인트 & 라우터
│   ├── v4_safe_crawler.py # 핵심 크롤러 로직 (The Engine)
│   ├── init_opensearch.py # 인덱스 매핑 & 분석기 설정
│   └── cache.py           # 캐싱 유틸리티
├── docker-compose.yml     # 인프라 구성 (OpenSearch 등)
...
3.  **Analytics Dashboard**: **Kibana**를 연동하여 브랜드 점유율, 가격 추이 등을 시각화합니다.
