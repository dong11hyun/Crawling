...
## 🚀 새로운 방향 (v4) - 안전 & 실시간 하이브리드 크롤러

### 핵심 목표
1. **안정성 (Stability)**: Rate Limit, User-Agent 로테이션, 429 에러 핸들링
2. **데이터 보존 (Persistence)**: 1건마다 즉시 저장(JSONL), 중단 시 재개 가능
3. **실시간성 (Real-time)**: 수집 즉시 OpenSearch 적재 및 검색 가능
4. **확장성 (Scalability)**: 상품 목록 API + Next.js 데이터 파싱 (v3 계승)
...
```
┌──────────────────────────────────────────┐
│ [Crawler] src/v4_safe_crawler.py         │
│ 1. 상품 목록 API 호출 (Batch 60)         │
│ 2. 상세 페이지 HTML 파싱 (__NEXT_DATA__) │
└──────────────────────────────────────────┘
           │ (1건마다 실시간 전송)
           ▼
┌──────────────────────┐      ┌──────────────────────┐
│ [Disk] 점진적 저장   │      │ [OpenSearch]         │
│ data/*.jsonl (백업)  │      │ musinsa_products     │
└──────────────────────┘      └──────────────────────┘
                                  │ (실시간 검색)
                                  ▼
┌──────────────────────┐      ┌──────────────────────┐
│ [API Server]         │      │ [Frontend]           │
│ FastAPI (Port 8000)  │ ◀─── │ index.html           │
└──────────────────────┘      └──────────────────────┘
```
...
| 기능 | v3 (Fast) | **v4 (Safe & Real-time)** |
|------|-----------|---------------------------|
| **속도** | 매우 빠름 (제한 없음) | **안전 속도 (1~2초/건)** |
| **차단 위험** | 높음 (Rate Limit 미준수) | **낮음 (랜덤 딜레이, UA회피)** |
| **데이터 저장** | 완료 후 일괄 저장 | **1건마다 실시간 저장 (JSONL)** |
| **검색 연동** | 수동 적재 필요 | **자동/실시간 적재** |
| **재개 기능** | 없음 (처음부터 다시) | **있음 (중단 지점부터)** |
...
### 4. 접속
- **웹 검색 UI**: `frontend/index.html` (브라우저로 열기)
...
| 파일 | 설명 | 비고 |
|------|------|------|
| `src/v4_safe_crawler.py` | 🛡️ **핵심 크롤러** (안전, 재개, 실시간) | Main |
| `src/api_server.py` | 🔍 검색 API (FastAPI) | Server |
| `src/init_opensearch.py` | ⚙️ 인덱스/분석기 설정 | Utils |
| `frontend/index.html` | 🖥️ 검색 웹 UI | Client |
| `data/crawl_progress_*.jsonl` | 💾 실시간 백업 데이터 | Data |
...
- **2026.01.28** - 🛡️ **v4 안전/실시간 크롤러 & 파이프라인 구축**