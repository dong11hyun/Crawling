...
### 4.0. 딥 크롤링: 사업자 정보 추출 (Deep Crawling) [핵심]

단순 상품 정보(제목, 가격)만 수집하는 일반 크롤러와 달리, **상세 페이지 내부의 사업자 정보까지 완전히 추출**합니다.

| 추출 항목 | 설명 | 활용 예시 |
|----------|------|----------|
| 상호명 | 판매 업체명 | 브랜드/셀러 분석 |
| 대표자명 | CEO 이름 | 신뢰도 검증 |
| 사업자등록번호 | 10자리 고유 번호 | 국세청 조회 연동 |
| 통신판매업 신고번호 | 온라인 판매 허가 | 정상 업체 여부 판별 |
| 연락처/이메일 | 고객센터 정보 | B2B 리드 생성 |
| 사업장 주소 | 실제 위치 | 지역별 분석 |

**기술적 구현**: Next.js 기반 SPA의 `__NEXT_DATA__` 스크립트 태그에서 서버 사이드 렌더링 데이터를 JSON으로 파싱하여 추출합니다.

```python
# v5_fast_crawler.py - 핵심 추출 로직
...
#### 💡 왜 브라우저(Selenium) 대신 스레드인가?

| 항목 | Chrome 탭 (Selenium) | 이 프로젝트 (ThreadPool) |
|------|---------------------|------------------------|
| **아키텍처** | 멀티프로세스 (탭마다 별도 프로세스) | 단일프로세스 + 멀티스레드 |
| **메모리** | 탭당 ~150MB | 스레드당 ~2MB |
| **4개 병렬 기준** | ~1.1GB | ~84MB |
| **렌더링** | 전체 (HTML→CSS→JS→이미지) | 없음 (HTML 텍스트만) |
| **속도** | 건당 3~5초 | 건당 0.2~0.3초 |

*   **I/O 바운드 최적화**: 네트워크 요청 대기 시간(1~2초)이 대부분이므로, Python GIL의 영향 없이 스레드가 효율적으로 동작합니다.
*   **결론**: 무신사는 API가 있어 브라우저가 불필요하고, 네트워크 I/O가 병목이라 **스레드 기반 경량 아키텍처**가 최적입니다.
